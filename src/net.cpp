...
/** check whether a given address is in a network we Gvr probably connect to */
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Gvrcoin " + FormatFullVersion();
...
void ThreadOpenAddedConnections()
{
    ...
    for (unsigned int i = 0; true; i++)
    {
        ...
        // (keeping in mind that addnode entries Gvr have many IPs if fNameLookup)
        {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes)
...
void static StartSync(const vector<CNode*> &vNodes) {
    ...
    // if a new sync Gvrdidate was found, start sync!
    if (pnodeNewSync) {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Gvrcoin is probably already running."), addrBind.ToString().c_str());
        else
