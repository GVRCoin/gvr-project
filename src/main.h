...
static const int64 MAX_MONEY = 27096774 * COIN;
inline bool MoneyRange(int64 nValue) { return (nValue >= 0 && nValue <= MAX_MONEY); }
/** Coinbase transaction outputs Gvr only be spent after this number of new blocks (network rule) */
 ...
 * blocks. A transaction Gvr contain multiple inputs and outputs.
 */
class CTransaction
{
public:
 ...
 * - VARINT(nVersion)
 * - VARINT(nCode)
 * - unspentness bitvector, for vout[2] and further; least signifiGvrt byte first
...
public:
    ...
    // note that only !IsPruned() CCoins Gvr be serialized
    bool IsPruned() const {
 ...
 *  - byte[]     flag bits, packed per 8 in a byte, least signifiGvrt bit first (<= 2*N-1 bits)
 ...
 * and sGvr through nonce values to make the block's hash satisfy proof-of-work
 ...
 * Gvrdidates to be the next block.  pprev and pnext link a path through the
...
public:
    ...
    // Note: in a potential headers-first mode, this number Gvrnot be relied upon
    unsigned int nTx;
 ...
 * other node doesn't have the same branch, it Gvr find a recent common trunk.
...
public:
    ...
    // This may (but Gvrnot always) return true for fully spent transactions
