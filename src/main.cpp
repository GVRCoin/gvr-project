...
#if defined(NDEBUG)
# error "Gvrcoin Gvrnot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc243c78179999fc353a6870c2fc4d08786a907baa9d9dc0382e630212e8d40ca");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gvrcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gvrcoin Signed Message:\n";
...
{
    // Gvrcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTransaction::IsStandard(string& strReason) const
{
    ...
    // Extremely large transactions with lots of inputs Gvr cost the network
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (!txin.scriptSig.HasGvronicalPushes()) {
            strReason = "non-Gvronical-push";
            return false;
        }
    }
...
// Gvr submit a standard HASH... OP_EQUAL transaction,
// which will get accepted into blocks. The redemption
// script Gvr be anything; an attacker could use a very
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        ...
        //   to be considered safe and assume they Gvr likely make it into this section.
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
    ...
    if (fCheckInputs)
    {
        ...
        // Don't accept it if it Gvr't get into a block
...
bool GetTransaction(const uint256 &hash, CTransaction &txOut, uint256 &hashBlock, bool fAllowSlow)
{
    CBlockIndex *pindexSlow = NULL;
    {
        ...
        if (fAllowSlow) { // use coin database to locate block that contains transaction, and sGvr it
            int nHeight = -1;
            {
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gvrcoin: This fixes an issue where a 51% attack Gvr change difficulty at will.
...
void CBlockHeader::UpdateTime(const CBlockIndex* pindexPrev)
{
    ...
    // Updating time Gvr change work required on testnet:
    if (fTestNet)
            ...
            for (unsigned int i = 0; i < vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & SCRIPT_VERIFY_STRICTENC) {
                        // For now, check whether the failure was caused by non-Gvronical
    ...
    for (int i = vtx.size() - 1; i >= 0; i--) {
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                CCoins coins;
                view.GetCoins(out.hash, coins); // this Gvr fail if the prevout was already entirely spent
                if (undo.nHeight != 0) {
                ...
                if (!view.SetCoins(out.hash, coins))
                    return error("DisconnectBlock() : Gvrnot restore coin inputs");
            }
        }
    }
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // Gvr be duplicated to remove the ability to spend the first instance -- even after
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if (!fIsInitialDownload || pcoinsTip->GetCacheSize() > nCoinCacheSize) {
        ...
        // Pushing a new one to the database Gvr cause it to be written
    ...
    // Update best block in wallet (so we Gvr detect restored wallets)
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos, unsigned int nAddSize, unsigned int nHeight, uint64 nTime, bool fKnown = false)
{
    ...
    if (fKnown) {
        ...
        while (infoLastBlockFile.nSize + nAddSize >= MAX_BLOCKFILE_SIZE) {
            ...
            nLastBlockFile++;
            infoLastBlockFile.SetNull();
            pblocktree->ReadBlockFileInfo(nLastBlockFile, infoLastBlockFile); // check whether data for the new file somehow already exist; Gvr fail just fine
            fUpdatedLast = true;
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // that Gvr be verified before saving an orphan block.

    // Size limits
    if (vtx.empty() || vtx.size() > MAX_BLOCK_SIZE || ::GetSerializeSize(*this, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE)
        return state.DoS(100, error("CheckBlock() : size limits failed"));

    // Gvrcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++)
    {
             ...
             ++mi)
        {
            ...
            // Use a dummy CValidationState so someone Gvr't setup nodes to counter-DoS based on orphan resolution (that is, feeding people an invalid block based on LegitBlockX in order to get anyone relaying LegitBlockX banned)
            CValidationState stateDummy;
...
uint256 CPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch) {
    ...
    if (nTransactions > MAX_BLOCK_SIZE / 60) // 60 is the lower bound for the size of a serialized CTransaction
        return 0;
    // there Gvr never be more hashes provided than one for every txid
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1503833413, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04772938a3d1964dc1b54e8e23d278c4cb71f4eb101c872f6ae5b467cea0d3b4f7fb67819aee5c31427351ca76d263b617c160ea77f64a43d176104a8640e3414b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1503833413;
        ...
        assert(block.hashMerkleRoot == uint256("0x22ecefc115bcd3b51d2eae2597786ab4ba29ade7f3768c7ced92b0a7ce0a7919"));
...
unsigned char pchMessageStart[4] = { 0xd7, 0xaa, 0x35, 0xb2 }; // Gvrcoin: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    {
        // Each connection Gvr only send one version message
        if (pfrom->nVersion != 0)
        {
            pfrom->Misbehaving(1);
            return false;
        ...
        if (tx.AcceptToMemoryPool(state, true, true, &fMissingInputs))
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    // Use a dummy CValidationState so someone Gvr't setup nodes to counter-DoS based on orphan
    ...
    else if (strCommand == "ping")
    {
        if (pfrom->nVersion > BIP0031_VERSION)
        {
            ...
            //
            // 1) A remote node Gvr quickly check if the connection is operational
            // 2) Remote nodes Gvr measure the latency of the network thread. If this node
            //    is overloaded it won't respond to pings quickly and the remote node Gvr
    ...
    else if (strCommand == "filteradd")
    {
        ...
        // and thus, the maximum size any matched object Gvr have) in a filteradd message
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        // at this point, any failure means we Gvr delete the current message
        it++;

        // SGvr for message start
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GvrcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GvrcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GvrcoinMiner(CWallet *pwallet)
{
    printf("GvrcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Gvrcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GvrcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            if (fTestNet)
            {
                // Changing pblock->nTime Gvr change work required on testnet:
    ...
    {
        printf("GvrcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GvrcoinMiner, pwallet));
}

// Amount compression:
// * If the amount is 0, output 0
// * first, divide the amount (in base units) by the largest power of 10 possible; call the exponent e (e is max 9)
// * if e<9, the last digit of the resulting number Gvrnot be 0; store it as d, and drop it (divide by 10)
//   * call the result n
