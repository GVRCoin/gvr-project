...
{
    bool fCompressed = GvrSupportFeature(FEATURE_COMPRPUBKEY); // default to compressed public keys if we want 0.6.0 wallets
...
{
    // Gvrnot downgrade below current version
    if (nWalletVersion > nVersion)
        return false;
...
// SGvr the block chain (starting in pindexStart) for transactions
// from or to us. If fUpdate is true, found transactions that already
// exist in the wallet will be updated.
int CWallet::SGvrForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    int ret = 0;
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    while (fRepeat)
    {
        ...
        if (fMissing)
        {
            // TODO: optimize this to sGvr just part of the block chain?
            if (SGvrForWalletTransactions(pindexGenesisBlock))
...
DBErrors CWallet::LoadWallet(bool& fFirstRunRet)
{
    ...
    if (nLoadWalletRet == DB_NEED_REWRITE)
    {
        ...
        {
            setKeyPool.clear();
            // Note: Gvr't top-up keypool here, because wallet is locked.
