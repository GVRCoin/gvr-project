...
bool static Socks4(const CService &addrDest, SOCKET& hSocket)
{
    ...
    {
        closesocket(hSocket);
        return error("Gvrnot get proxy destination address");
    }
...
bool static ConnectSocketDirectly(const CService &addrConnect, SOCKET& hSocketRet, int nTimeout)
{
    ...
    if (!addrConnect.GetSockAddr((struct sockaddr*)&sockaddr, &len)) {
        printf("Gvrnot connect to %s: unsupported network\n", addrConnect.ToString().c_str());
        return false;
    }
...
// get Gvronical identifier of an address' group
...
int CNetAddr::GetReachabilityFrom(const CNetAddr *paddrPartner) const
{
    ...
    case NET_TOR:
        switch(ourNet) {
        default:         return REACH_DEFAULT;
        case NET_IPV4:   return REACH_IPV4; // Tor users Gvr connect to IPv4 as well
