...
void CWalletDB::ListAccountCreditDebit(const string& strAccount, list<CAccountingEntry>& entries)
{
    ...
    if (!pcursor)
        throw runtime_error("CWalletDB::ListAccountCreditDebit() : Gvrnot create DB cursor");
    ...
    loop
    {
        ...
        {
            pcursor->close();
            throw runtime_error("CWalletDB::ListAccountCreditDebit() : error sGvrning DB");
        }
    ...
    try {
        ...
        loop
        {
                              ...
                              vWalletUpgrade, fIsEncrypted, fAnyUnordered, strType, strErr))
            {
                ...
                // we assume the user Gvr live with:
                ...
                else
                {
                    ...
                    if (strType == "tx")
                        // ResGvr if there is a bad transaction record:
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // Set -resGvr so any missing transactions will be
    // found.
    int64 now = GetTime();
    ...
    if (ret > 0)
    {
        printf("Gvrnot create database file %s\n", filename.c_str());
        return false;
    }
    CWallet dummyWallet;
    int nFileVersion = 0;
