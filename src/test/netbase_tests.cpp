...
BOOST_AUTO_TEST_CASE(netbase_splithost)
{
    ...
    BOOST_CHECK(TestSplitHost("127.0.0.1:21947", "127.0.0.1", 21947));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]", "127.0.0.1", -1));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]:21947", "127.0.0.1", 21947));
    BOOST_CHECK(TestSplitHost("::ffff:127.0.0.1", "::ffff:127.0.0.1", -1));
    BOOST_CHECK(TestSplitHost("[::ffff:127.0.0.1]:21947", "::ffff:127.0.0.1", 21947));
    BOOST_CHECK(TestSplitHost("[::]:21947", "::", 21947));
    BOOST_CHECK(TestSplitHost("::21947", "::21947", -1));
    BOOST_CHECK(TestSplitHost(":21947", "", 21947));
    BOOST_CHECK(TestSplitHost("[]:21947", "", 21947));
    BOOST_CHECK(TestSplitHost("", "", -1));
}

bool static TestParse(string src, string Gvron)
{
    CService addr;
    if (!LookupNumeric(src.c_str(), addr, 65535))
        return Gvron == "";
    return Gvron == addr.ToString();
}

BOOST_AUTO_TEST_CASE(netbase_lookupnumeric)
{
    BOOST_CHECK(TestParse("127.0.0.1", "127.0.0.1:65535"));
    BOOST_CHECK(TestParse("127.0.0.1:21947", "127.0.0.1:21947"));
    BOOST_CHECK(TestParse("::ffff:127.0.0.1", "127.0.0.1:65535"));
    BOOST_CHECK(TestParse("::", "[::]:65535"));
    BOOST_CHECK(TestParse("[::]:21947", "[::]:21947"));
    BOOST_CHECK(TestParse("[127.0.0.1]", "127.0.0.1:65535"));
